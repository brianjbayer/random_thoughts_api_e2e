...
simply use the `docker compose up` command...
...
Use the `-l` local option specifying
`random_thoughts_api` as the argument with the
`dockercomposerun` script.

For example to use the latest E2E tests interactively...

```
./script/dockercomposerun -l random_thoughts_api bash
```

Then, in another terminal, you can `docker exec` into
your local `random_thoughts_api` Development container
and operate it (e.g. start the server)...

```
docker exec -it random_thoughts_api bash
```
